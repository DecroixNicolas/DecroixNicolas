<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Nicolas Decroix | Portfolio</title>
  <style>
    body {
      font-family: Arial, sans-serif;
      margin: 0;
      background: #ffffff;
      color: #111;
      line-height: 1.6;
    }

    .container {
      max-width: 800px;
      margin: 0 auto;
      padding: 60px 20px;
    }

    header {
      margin-bottom: 60px;
    }

    h1 {
      font-size: 40px;
      margin: 0;
    }

    h2 {
      margin-top: 40px;
      font-size: 24px;
    }

    .role {
      font-size: 18px;
      color: #444;
      margin-top: 8px;
    }

    .links a {
      margin-right: 15px;
      color: #000;
      text-decoration: underline;
    }

    .project {
      margin-bottom: 30px;
    }

    .project-title {
      font-weight: bold;
      font-size: 18px;
    }

    .project-links a {
      margin-right: 10px;
      font-size: 14px;
    }

    ul {
      padding-left: 20px;
    }

    .section {
      margin-bottom: 50px;
    }

    footer {
      margin-top: 60px;
      font-size: 14px;
    }
  </style>
</head>

<body>
  <div class="container">

    <!-- HERO -->
    <header>
      <h1>Nicolas Decroix</h1>
      <div class="role">Diplomé Master II Géopolitique | Spécialisé Défense/OTAN | Sécurité Flanc Est et Arctique</div>
      <p>
        I build fast, clean, and user-focused web applications.
      </p>

      <div class="links">
        <a href="mailto:decroix.nicolasfrancois@gmail.com">Email</a>
        <a href="CV DECROIX Nicolas.pdf" target="_blank">CV</a>
      </div>
    </header>

    <!-- PROJECTS -->
    <div class="section">
      <h2>Projects</h2>

      <div class="project">
        <div class="project-title">Mémoire de Master 1</div>
        <p>La Pologne et l'exclave de Kaliningrad : projet Bouclier Oriental et nouvelles réalités 
frontalières dans le contexte sécuritaire de la guerre en Ukraine.</p>
        <ul>
          <li>Travail de recherche continu sur l'année de M1 combinant études théoriques, collecte d'informations en sources ouvertes et revues de presse.
          <li>Terrain de recherche d'un mois en Pologne, entre Varsovie, Olsztyn et la frontière russo-polonaise. Rencontres et interviews en anglais et polonais avec des représentants locaux (maires, administrateurs régionaux), universitaires et habitants de la zone.
          <li>Création de représentations cartographiques originales afin d'illustrer mes recherches.
          <li>Aboutissement de ce travail de recherche en un mémoire de 120 pages, obtenant la note de 17/20
          <li> </li>
        </ul>
        <div class="project-links">
          <a href="#">Live</a>
          <a href="#">Code</a>
        </div>
      </div>

      <div class="project">
        <div class="project-title">Weather Dashboard</div>
        <p>Real-time weather app using external API.</p>
        <ul>
          <li>API integration with dynamic UI</li>
          <li>Responsive design</li>
          <li>Optimized loading performance</li>
        </ul>
        <div class="project-links">
          <a href="#">Live</a>
          <a href="#">Code</a>
        </div>
      </div>

    </div>

    <!-- SKILLS -->
    <div class="section">
      <h2>Skills</h2>
      <p>
        <strong>Languages:</strong> JavaScript, Python<br />
        <strong>Frontend:</strong> React, HTML, CSS<br />
        <strong>Tools:</strong> Git, Figma, VS Code
      </p>
    </div>

    <!-- ABOUT -->
    <div class="section">
      <h2>About</h2>
      <p>
        Computer Science graduate focused on frontend development and UI design.
        I enjoy building practical tools and improving user experience.
        Currently looking for junior developer roles.
      </p>
    </div>

    <!-- CONTACT -->
    <footer>
      <h2>Contact</h2>
      <p>Email: you@email.com</p>
      <p>GitHub: github.com/yourusername</p>
    </footer>

  </div>
</body>
</html>
<!--
git add .
git commit -m "Mise à jour du code"
git push origin main


- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
